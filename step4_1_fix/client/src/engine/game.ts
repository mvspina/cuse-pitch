...
export const suitSymbol: Record<Suit, string> = { S: '♠', H: '♥️', D: '♦️', C: '♣' }
...
export function buildPlayersTeams(settings: GameSettings, names?: string[]): { players: Player[], teams: Team[] } {
  ...
  if (pc === 2) {
    teams.push({ id: 'A', name: 'Team A', score: 0 }, { id: 'B', name: 'Team B', score: 0 })
...
function applyHandScoring(state: GameState, result: HandResult): { teams: Team[], winnerTeamId: string | null } {
  ...
  if (bidderTeam) {
    if (result.stmFailed) bidderTeam.score -= result.stmPenalty
    else if (result.bidderSetPenalty > 0) bidderTeam.score -= result.bidderSetPenalty
  }
...
export function reducer(state: GameState, action: Action): GameState {
  ...
  if (action.type === 'CONFIRM_DISCARD') {
    ...
    const next = done.every(Boolean) ? state.dealerIndex : (pi + 1) % state.players.length
  ...
  if (action.type === 'PLAY_CARD') {
    ...
    if (plays.length === pc) {
      ...
      out = { ...out, tricksWonByTeamId, cardsCapturedByTeamId }
